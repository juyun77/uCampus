...
public class SecDAO {
	...
	public SecDAO() {
	 ...
	 public ArrayList<Sec> getstuSec2(String userID) { 
		  String SQL ="SELECT * FROM Section WHERE secName NOT IN (SELECT shsSecName FROM Section_Has_Student WHERE shsUserID = ?)";
		  ArrayList<Sec> list = new ArrayList<Sec>();
		  try {
		  pstmt = conn.prepareStatement(SQL); //prepared statement 의 어떤 준비된 문장을
		  pstmt.setString(1, userID); // 가장 중요한데, 기본적으로
		  ...
		  while (rs.next()) {
				Sec sec1 = new Sec();
				sec1.setSecName(rs.getString(1));
				sec1.setSecDaysTime(rs.getString(2));
				sec1.setSecCoCode(rs.getString(3));
				sec1.setSecUserName(rs.getString(4));
				list.add(sec1);
			}
		} catch (Exception e) {
			e.printStackTrace();
